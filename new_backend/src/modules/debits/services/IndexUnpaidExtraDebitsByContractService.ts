...
export default class IndexDebitsByContractService {
    ...
    public async execute(contract_id: string): Promise<Debit[] | []> {
        ...
        if (!contract) {
            throw new AppError(
                'não é possível listar os débitos adicionais de um contrato inexistente!',
            );
        }

        if (contract.status !== 'active') {
            throw new AppError(
                'não é possível listar os débitos adicionais de um contrato que não está ativo!',
            );
        }

        const debits = await this.debitsRepository.findUnpaidExtraByContract(
            contract_id,
        );
