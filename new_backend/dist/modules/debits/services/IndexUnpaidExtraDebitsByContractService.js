...
let IndexDebitsByContractService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute(contract_id) {
    ...
    if (!contract) {
      throw new _AppError.default('não é possível listar os débitos adicionais de um contrato inexistente!');
    }

    if (contract.status !== 'active') {
      throw new _AppError.default('não é possível listar os débitos adicionais de um contrato que não está ativo!');
    }

    const debits = await this.debitsRepository.findUnpaidExtraByContract(contract_id);
    return debits;
  }
