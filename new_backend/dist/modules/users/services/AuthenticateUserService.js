...
let AuthenticateUserService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if (!user) {
      throw new _AppError.default('Credenciais incorretas!', 401);
    }

    const passwordsMatches = await this.hashProvider.compare(password, user.password);
    ...
    const token = (0, _jsonwebtoken.sign)(payload, secret, {
      subject: user.id,
      expiresIn
    });
    return {
