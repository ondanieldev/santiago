...
let DiskReceiptProvider = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async generate({
    ...
    const itemsArray = items.map(item => {
      ...
      totalValue += subtotal;
      return [item.description.toString(), item.quantity.toString(), `R$ ${item.base_value.toFixed(2).toString().replace('.', ',')}`, item.is_compound_variation ? `${item.variation}p. a.m.` : `${item.variation}p.`, `R$ ${subtotal.toFixed(2).toString().replace('.', ',')}`];
    });
    ...
    const documentDefinition = {
      ...
      content: ['*******************************************', 'COLEGIO SANTIAGO', 'RUA RIO GRANDE DO SUL, 863', 'BAIRRO ESPIRITO SANTO - MG', '-------------------------------------------', {
        ...
        text: `RESPONSÁVEL FIN.: ${(0, _formatFunctions.returnFirstName)(client.name)}`,
        alignment: 'left'
      }, {
        text: `CPF: ${(0, _formatFunctions.returnCPFWithMask)(client.cpf)}`,
        alignment: 'left'
      }, '-------------------------------------------', {
        text: `RECEBIDO POR: ${(0, _formatFunctions.returnFirstName)(operative.name)}`,
        alignment: 'left'
      }, {
        columns: [{
          text: `DATA: ${date} `,
    ...
    await _fs.default.promises.rename(_path.default.resolve(_upload.default.tmpFolder, filename), _path.default.resolve(_upload.default.uploadFolder, filename));
    return filename;
  }
